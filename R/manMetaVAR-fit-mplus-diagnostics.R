...
#' Monte Carlo diagnostics from the posterior draws saved by `FitMplus()`.
#'
...
#'
#' @return An object of class `manmetavar.mplus.diagnostics` containing a
...
#' @export
FitMplusDiagnostics <- function(object,
                                burnin = NULL,
                                level = 0.95) {
  if (
    length(level) != 1L ||
  ...
  if (!is.null(object$args$default_priors)) {
    default_priors <- isTRUE(object$args$default_priors)
  } else {
    default_priors <- !any(
      grepl(
  ...
  if (inherits(object$elapsed, "difftime")) {
    ...
    elapsed_seconds <- as.numeric(object$elapsed)
  }
  run <- data.frame(
    default_priors = default_priors,
    chains = length(chain),
    requested_iterations = as.integer(object$args$iter),
    retained_iterations_per_chain = n_iterations,
    burnin = attr(draws, "burnin"),
    retained_draws = nrow(draws),
    level = level,
    elapsed_seconds = elapsed_seconds,
    max_rhat = safe_max(parameters$rhat),
    min_ess_bulk = safe_min(parameters$ess_bulk),
    min_ess_tail = safe_min(parameters$ess_tail),
    n_rhat_na = sum(!is.finite(parameters$rhat)),
    n_rhat_gt_1_01 = sum(parameters$rhat > 1.01, na.rm = TRUE),
    n_ess_bulk_na = sum(!is.finite(parameters$ess_bulk)),
    n_ess_bulk_lt_100 = sum(parameters$ess_bulk < 100, na.rm = TRUE),
    n_ess_bulk_lt_200 = sum(parameters$ess_bulk < 200, na.rm = TRUE),
    n_ess_bulk_lt_400 = sum(parameters$ess_bulk < 400, na.rm = TRUE),
    n_ess_tail_na = sum(!is.finite(parameters$ess_tail)),
    n_ess_tail_lt_100 = sum(parameters$ess_tail < 100, na.rm = TRUE),
    n_ess_tail_lt_200 = sum(parameters$ess_tail < 200, na.rm = TRUE),
    n_ess_tail_lt_400 = sum(parameters$ess_tail < 400, na.rm = TRUE),
    row.names = NULL,
    check.names = FALSE
  )
  out <- list(
...
#' @noRd
#' @keywords internal
.PrintMplusDiag <- function(x,
                            digits = 4,
                            ...) {
  cat("FitMplus posterior diagnostics\n\n")
  print(x$run, row.names = FALSE)
