data_process_population_k4 <- function(overwrite = FALSE,
                                       n_population = 1000000L,
                                       batch_size = 50000L,
                                       seed = 42001L,
                                       margin = 1) {
  cat("\ndata_process_population_k4\n")
  if (
  ...
  if (write) {
    ...
    if (!file.exists(model_file)) {
      stop(
        paste0(
          "`data/modelk4.rda` does not exist. ",
          "Run data-process-model-k4 first."
        ),
        call. = FALSE
      )
    }
    ...
    modelk4 <- data_environment$modelk4
    ...
    if (k != 4L) {
      stop(
        "`data/modelk4.rda` must contain a modelk4 with `k = 4`.",
        call. = FALSE
      )
    }
    ...
    calibrate_beta <- function(heterogeneity,
      ...
      for (j in seq_len(n_beta_pairs)) {
        ...
        gradient[row] <-
          gradient[row] - beta_mean[col]

        gradient[col] <-
          gradient[col] - beta_mean[row]
      ...
      beta_mu <- matrix(
        ...
        nrow = k,
        ncol = k,
        dimnames = dimnames(modelk4$beta_mu)
      )
    ...
    for (i in seq_along(heterogeneity)) {
      ...
      condition$mu_mu <- modelk4$mu_mu
      condition$mu_sigma <- modelk4$mu_sigma
      condition$ma_fixed <- ma_fixed
      condition$ma_random <- ma_random
      condition$parameter <- parameter
      condition$nominal_beta_mu <- modelk4$beta_mu
      condition$nominal_beta_sigma <-
        heterogeneity[i] * modelk4$beta_sigma
    ...
    class(populationk4) <- c(
      "manmetavar.populationk4",
      class(populationk4)
    )
